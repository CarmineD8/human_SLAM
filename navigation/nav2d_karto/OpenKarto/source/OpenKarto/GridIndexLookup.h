...
namespace karto
{
  ...
  public:
     ...
     */
    ///The angleCenter is actually the heading of the laser scan
    void ComputeOffsets(LocalizedLaserScan* pScan, kt_double angleCenter, kt_double angleOffset, kt_double angleResolution)
    {
      ...
      ///A list of vectors, each vector represent the end point of the laser ray at each angle resolution
      ...
      {
        /// do inverse transform to get points in local coordinates(wrt to the scan pose). localPoints has the same size of PointReadings
        Pose2 vec = transform.InverseTransformPose(Pose2(*iter, 0.0));
        localPoints.Add(vec);
      }

      //////////////////////////////////////////////////////
      /// create lookup array for different angles, each angleIndex corresponds to an angle
      /// for each angle there's an array to look up
      kt_double angle = 0.0;
  ...
  private:
    ...
    {
      ///Each lookupArray has a size of the range readings
      ...
      ///The m_pGRid is actually the correlation grid
      /// get the offset of this correlation grid means the coordinate of the left-lower corner
      ...
      ///This pointer starts pointing at the first position of this lookup array
      kt_int32s* pAngleIndexPointer = m_ppLookupArray[angleIndex]->GetArrayPointer();
      ///Start search for each readings
      karto_const_forEach(Pose2List, &rLocalPoints)
      {
        ///Get the local coordinates in position of this scan reading wrt to the scan pose
        /// (for the localpoints, its the origin)
        ...
        /// offset+rGridOffset is grid coordinate of the point after the reading rotate angle
        ...
        ///The variables in the pAngleIndexPoniter is just the index get from the offset
  ...
  private:
    Grid<T>* m_pGrid;  ///Point to a 2-dimensional grid, here is it point to the correlation grid

    kt_int32u m_Capacity;
    kt_int32u m_Size; ///Size is the total number of angles in this lookup table
    
    LookupArray **m_ppLookupArray; ///It's a double pointer, point to a position in an array at an indicate angle

    // for sanity check
    List<kt_double> m_Angles;  ///List of angles, the size of the list should be m_Size, the list of angles for the whole search table
  }; // class GridIndexLookup
