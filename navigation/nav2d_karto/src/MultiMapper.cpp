...
MultiMapper::MultiMapper()
{
	...
	robotNode.param("customer_order", mCustomInputTopic, std::string("put_node"));
	...
	if(mState == ST_MAPPING)
	{
        ...
        if(!mCustomerOrder){
            ...
            if (success) {
                ...
                bool ok = true;
                try {
                    mTransformListener.transformPose(mOffsetFrame, tf::Stamped<tf::Pose>(map_in_robot, ros::Time(
                            0) /*scan->header.stamp*/, mLaserFrame), map_in_odom);
                }
                catch (tf::TransformException e) {
                ...
                if (ok) {
                    mMapToOdometry = tf::Transform(tf::Quaternion(map_in_odom.getRotation()),
                                                   tf::Point(map_in_odom.getOrigin())).inverse();
            ...
            if(!imageObject->HasCustomItem()) return;
...
//	rosScan.scan = *scan;
    mScanPublisher.publish(rosScan);
    std::cout<<"Sent this specail scan."<<std::endl;
}
